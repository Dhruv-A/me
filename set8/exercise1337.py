...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    return '🔥' + '🔥'.join(list(input_string.upper())) + '🔥'
...
def best_letter_for_pets() -> str:
    ...
    for letter in the_alphabet:
        curr_num_occur = len(pet_filter(letter))
        if curr_num_occur > max_num_occur:
            max_num_occur = curr_num_occur
...
def make_filler_text_dictionary() -> dict:
    ...
    for length in range(3, 8):
        wd[length] = []
        for _ in range(4):
            wd[length].append(requests.get(f'{url}{length}').text)
    return wd
...
def random_filler_text(number_of_words=200) -> str:
    ...
    for _ in range(number_of_words):
        words.append(random.choice(my_dict[random.randint(3, 7)]))
...
def fast_filler(number_of_words=200) -> str:
    ...
    cleaned_dict = {}
    for key in my_dict:
        cleaned_dict[int(key)] = my_dict[key]

    words = []
    for _ in range(number_of_words):
        words.append(random.choice(cleaned_dict[random.randint(3, 7)]))
